...
const postgresConfig = {
  ...
  user: 'RCC', // replace this with your username
...
/* -----------------------------------------
   ...
   @output: [{id, title, author_id, subject_id}]
*/
const firstTenBooks = db.any('select * from books limit 10')  // = .... IMPLEMENT THIS FUNCTION
...
findAuthorsOrderedByLastName.then(authors => {
  ...
  assert.deepEqual(authors[20].last_name, 'Worsley')
}).catch(error => {
  console.log('Whoops, my function doesnt behave as expected.', error);
});
// /* --------End of Exercise 3---------------- */
...
// */
let findBookAuthors = db.any('SELECT first_name, last_name, title FROM authors  INNER JOIN books ON authors.id = books.author_id')
  findBookAuthors.then(authors => {
    assert.equal(authors[0].first_name, 'John')
    assert.equal(authors[0].last_name, 'Worsley')
    assert.equal(authors[0].title, 'Practical PostgreSQL')
    return authors
}).catch(error => {
  console.log('Whoops, my function doesnt behave as expected.', error);
});
...
//       {author_id: 7805}]
// */
let authorIdWithTwoBooks =db.any('SELECT author_id FROM books') // change the sql
authorIdWithTwoBooks.then(authors => {
  assert.equal(authors[0].author_id, 1809)
  assert.equal(authors[1].author_id, 7805)
}).catch(error => {
console.log('Whoops, my function doesnt behave as expected.', error);
});
